...
"""
Tool to generate a VMs file to be used with pyCloudSim
"""
__version__ = "0.1"
...
class VMsGenerator():
    ...
    def load_trace_index(self, filename):
    ...
    def parse_args(self):
        ...
        self.infile = self.get_default_arg("traces-mean-30-45.txt", args.infile)
        self.outfile = self.get_default_arg("vms.json", args.outfile)

    def run(self):
        self.parse_args()
        f = self.load_trace_index(self.infile)
        vms = []
        for id in range(1, self.vmcount+1):
            trace = {}
            trace['cpu']  = self.clean_prefix(f.readline())
            trace['mem']  = self.clean_prefix(f.readline())
            trace['disk'] = self.clean_prefix(f.readline())
            trace['net']  = self.clean_prefix(f.readline())
